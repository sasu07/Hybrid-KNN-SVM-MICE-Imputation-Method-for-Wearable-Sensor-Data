...
class RealisticMissingDataGenerator:
    ...
    def _identify_sensor_groups(self):
        """Identify sensor groups based on column names"""
        sensor_groups = {
            'accelerometer': [col for col in self.data.columns if 'Accelerometer' in col],
            'gyroscope': [col for col in self.data.columns if 'Gyroscope' in col],
            'orientation': [col for col in self.data.columns if 'Orientation' in col],
            'magnitude': [col for col in self.data.columns if 'Magnitude' in col],
    ...
    def activity_dependent_failures(self, missing_rate):
        ...
        activity_sensor_map = {
            'Mers': self.sensor_groups['accelerometer'],  # Walking affects accelerometer
            'Alergare': self.sensor_groups['gyroscope'],  # Running affects gyroscope
            'Urcat': self.sensor_groups['orientation'],   # Climbing affects orientation
            'Coborat': self.sensor_groups['angles']       # Descending affects angles
        }
